...
{	
	...
	//相当于附加在文件尾
	//可以将结构体地址传给write函数，写进去。
	return(0);
}

int i_print_history(int len, int i)//被调用的时候len是一个恒为8的常量，i是负数，代表往回翻几页
{
	...
	if (i != 0)
	{
		position = len*i*HSTR_LEN;//代表往回翻i页后的指针位置。i本身是负的了，所以position也会是负值
		i_lseek(mainfd, position, SEEK_END);//指向文件尾后再偏position个单位
	}
	else
	{
		position = len*i*HSTR_LEN;
		//为什么此处还要计算一个position呢
		i_lseek(mainfd, HSTR_LEN, SEEK_SET);
		//将文件指针指向开头偏HSTR_LEN个量
	}
